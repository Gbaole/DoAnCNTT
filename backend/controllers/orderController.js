...
const myOrdersShipper = require('./mongoQuery/OrderQuery/myOrder/myOrdersShipper');
const myOrderUser = require('./mongoQuery/OrderQuery/myOrder/myOrdersUser');
const myOrderAdmin = require('./mongoQuery/OrderQuery/myOrder/myOrdersAdmin');
const { Product } = require('../models/product');
const myOrdersAdmin = require('./mongoQuery/OrderQuery/myOrder/myOrdersAdmin');
...
exports.newOrder = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (cart.products.length === 0) {
      return next(new ErrorHandler('Không có sản phẩm nào trong giỏ hàng.', 400, res));
    }
    for (let i = 0; i < cart.products.length; i++) {
      const prod = await Product.findById(cart.products[i].id, { price: 1, productType: 1 });
    ...
    console.log(error);
    res.status(400).json({ success: false, message: error.toString(), requestBody: req.body });
  }
});
...
exports.getSingleOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!order) {
    return next(new ErrorHandler(`No order not found with id: ${req.params.id}`, 404, res));
  }
...
exports.myOrders = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (req.user.role == 'shipper') {
      try {
        let result = await Order.aggregate(myOrdersShipper(filter, req.user._id, skip, limit));
      ...
      const result = await Order.aggregate(myOrderUser(filter, req.user._id, skip, limit));
      // console.log(result);
      if (result.length !== 0) {
        res.status(200).json({
          success: true,
...
exports.deleteOrder = catchAsyncErrors(async (req, res, next) => {
  const order = await Order.findOneAndDelete({ 'shippingInfo.mdh': req.params.id });
  if (!order) {
    return next(new ErrorHandler(`No order not found with id: ${req.params.id}`, 404));
  }
  await order.remove();
  res.status(200).json({
    success: true
...
async function upload2Cloudinary(path, mdh) {
  let res;
  await cloudinary.uploader
    .upload(path, { public_id: mdh })
    .then((data) => {
      // console.log(data);
